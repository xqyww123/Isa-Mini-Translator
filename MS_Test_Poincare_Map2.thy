...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
(File.read (Path.explode "./Translator/t10.txt"))\<close>
...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
(File.read (Path.explode "./Translator/t11.txt"))\<close>
...
mproof
define H where H_def: "H \<equiv> \<lambda>(x, y). (x, f (x, y))"
define H' where H'_def: "H' \<equiv> \<lambda>x. (embed1_blinfun o\<^sub>L fst_blinfun) + (embed2_blinfun o\<^sub>L (f' x))"
have f'_inv: "f' (x, y) o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using T1 T2
by (auto simp: invertibles_blinfun_def ac_simps intro!: exI[where x=T]) 
obtain d0 where e0: "0 < d0" "ball (f' (x, y) o\<^sub>L embed2_blinfun) d0 \<subseteq> invertibles_blinfun"
using openE[OF blinfun_inverse_open f'_inv]
by (auto) 
have fact0: "isCont (\<lambda>s. f' s o\<^sub>L embed2_blinfun) (x, y)"
apply (auto intro!: continuous_intros f'C)  .
have fact1: "\<forall>\<^sub>F s in at (x, y). f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using fact0[unfolded isCont_def, THEN tendstoD, OF \<open>0 < d0\<close>]
 apply (eventually_elim)
using e0
by (auto simp: subset_iff dist_commute) 
obtain e0 where fact2: "e0 > 0" "xa \<noteq> (x, y) \<Longrightarrow> dist xa (x, y) < e0 \<Longrightarrow>
        f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun" for xa
unfolding eventually_at
using fact1[unfolded "eventually_at"]
  by (auto) 

have e0: "e0 > 0" "dist xa (x, y) < e0 \<Longrightarrow> f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun" for xa
using fact2
 apply (-)
mproof
show "0 < e0 \<Longrightarrow> (\<And>xa. xa \<noteq> (x, y) \<Longrightarrow> dist xa (x, y) < e0 \<Longrightarrow> f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun) \<Longrightarrow> 0 < e0"
by (simp) 
mqed
using f'_inv
 apply (cases "xa = (x, y)")
...
using H' assms(2) has_derivative_linear
by (blast) 
have "*": "blinfun_apply T (blinfun_apply (f' (x, y)) (0, b)) = b" for b
using blinfun_inverseD[OF T2, of b]
by (simp) 
have fact3: "inj (f' (x, y) o\<^sub>L embed2_blinfun)"
by (metis (no_types, lifting) "*" blinfun_apply_blinfun_compose embed2_blinfun.rep_eq injI) 
have fact4[simp]: "blinfun_apply (f' (x, y)) (0, b) = 0 \<Longrightarrow> b = 0" for b
using fact3
 apply (subst (asm) linear_injective_0)
mproof
show "blinfun_apply (f' (x, y)) (0::'a, b) = (0::'c) \<Longrightarrow> linear (blinfun_apply (f' (x, y) o\<^sub>L embed2_blinfun))"
 apply (rule bounded_linear.linear)
 apply (rule blinfun.bounded_linear_right)
done
mqed
mproof
show "blinfun_apply (f' (x, y)) (0::'a, b) = (0::'c) \<Longrightarrow> \<forall>xa. blinfun_apply (f' (x, y) o\<^sub>L embed2_blinfun) xa = (0::'c) \<longrightarrow> xa = (0::'b) \<Longrightarrow> b = (0::'b)"
by (simp) 
mqed
done
have fact5: "inj (H' (x, y))"
 apply (subst linear_injective_0)
 apply (rule linear_H')
 apply (fact)
 apply (auto simp: H'_def blinfun.bilinear_simps zero_prod_def)
done
define Hi where Hi_def: "Hi = (embed1_blinfun o\<^sub>L fst_blinfun) + ((embed2_blinfun o\<^sub>L T o\<^sub>L (snd_blinfun - (f' (x, y) o\<^sub>L embed1_blinfun o\<^sub>L fst_blinfun))))"
...
have Hi: "Hi o\<^sub>L H' (x, y) = 1\<^sub>L"
 apply (auto simp: H'_def fun_eq_iff blinfun.bilinear_simps Hi_def intro!: ext blinfun_eqI)
...
using has_derivative_locally_injective_blinfun[OF S Hi H' cH']
by (auto) 
define r where r_def: "r = min r0 e0"
have r: "0 < r" "ball (x, y) r \<subseteq> S" and inj: "inj_on H (ball (x, y) r)" and r_inv: "\<And>s. s \<in> ball (x, y) r \<Longrightarrow> f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
mproof
show "0 < r"
using e0 r0
by (auto simp: r_def) 
mqed
mproof
show "ball (x, y) r \<subseteq> S"
using e0 r0
by (auto simp: r_def) 
mqed
mproof
show "inj_on H (ball (x, y) r)"
using inj
...
mproof
fix s :: "'a \<times> 'b"
show "s \<in> ball (x, y) r \<Longrightarrow> f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using e0 r0
by (auto simp: r_def dist_commute) 
mqed
done
obtain i :: 'a where fact6: "i \<in> Basis"
using nonempty_Basis
by (blast) 
define undef where undef_def: "undef \<equiv> (x, y) + r *\<^sub>R (i, 0)"
have ud: "\<not> dist (x, y) undef < r"
using \<open>r > 0\<close> \<open>i \<in> Basis\<close>
by (auto simp: undef_def dist_norm) 
define G where G_def: "G \<equiv> the_inv_into (ball (x, y) r) H"
{
fix u and v
assume fact8[simp]: "(u, v) \<in> H ` ball (x, y) r"
note fact9[simp] = inj
have fact10: "(u, v) = H (G (u, v))"
unfolding G_def
by (subst f_the_inv_into_f[where f=H]) (auto)
have fact11: "\<dots> = H (G (u, v))"
by (auto simp: G_def) 
have fact12: "\<dots> = (fst (G (u, v)), f (G (u, v)))"
by (auto simp: H_def split_beta') 
have fact13: "u = fst (G (u, v))" "v = f (G (u, v))"
using fact10 fact11 fact12
by (simp_all) 
have fact14: "f (u, snd (G(u, v))) = v" "u = fst (G (u, v))"
using fact13
...
have fact16: "(x, f (x, y)) \<in> H ` ball (x, y) (r/2)"
using \<open>r > 0\<close>
by (auto simp: H_def) 
obtain e' where e': "e' > 0" "cball (x, f (x, y)) e' \<subseteq> H ` ball (x, y) (r/2)"
using open_contains_cball[THEN iffD1, OF oH, rule_format, OF fact16]
by (auto) 
have inv_subset: "the_inv_into (ball (x, y) r) H a = the_inv_into R H a" if fact17: "a \<in> H ` R" "R \<subseteq> (ball (x, y) r)" for a and R
...
have GH: "G (H z) = z" if fact18: "dist (x, y) z < r" for z
by (auto simp: G_def the_inv_into_f_f inj fact18) 
define e where e_def: "e = min (e' / 2) e0"
define r2 where r2_def: "r2 = r / 2"
have r2: "r2 > 0" "r2 < r"
using \<open>r > 0\<close>
by (auto simp: r2_def) 
have fact19: "e > 0"
using e' e0
by (auto simp: e_def) 
have fact20: "cball x e \<times> cball (f (x, y)) e \<subseteq> H ` ball (x, y) (r/2)"
using cball_times_subset[of "x" e' "f (x, y)"] e'
by (force simp: e_def) 
have e_r_subset: "z \<in> cball x e \<Longrightarrow> (z, 0) \<in> H ` ball (x, y) (r/2)" for z
using fact20 \<open>0 < e\<close> assms(6)
by (auto simp: H_def subset_iff) 
have u0: "(u, 0) \<in> H ` ball (x, y) r" if fact21: "u \<in> cball x e" for u
...
using r2
by (auto) 
have G_r: "G (u, 0) \<in> ball (x, y) r" if fact22: "u \<in> cball x e" for u
unfolding G_def
 apply (rule the_inv_into_into)
 apply (fact)
 apply (auto)
...
note fact23 = e_r_subset
have G2: "f (x, snd (G (x, 0))) = 0" "snd (G (x, 0)) = y" "\<And>u. u \<in> cball x e \<Longrightarrow> f (u, snd (G (u, 0))) = 0" "continuous_on (cball x e) (\<lambda>u. snd (G (u, 0)))" "(\<lambda>t. (t, snd (G (t, 0)))) ` cball x e \<subseteq> S" "e > 0" "((\<lambda>u. snd (G (u, 0))) has_derivative (\<lambda>u. snd (Hi (u, 0)))) (at x)"
using uv fact15 fact23
 apply (auto simp: G_def split_beta' intro!: continuous_intros continuous_on_compose2[OF cH])
mproof
assume prems: "\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v" "f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c)" "\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)"
show "snd (the_inv_into (ball (x, y) r) H (x, 0::'c)) = y"
proof (-) 
have fact24: "the_inv_into (ball (x, y) r) H (x, 0) = (x, y)"
...
mproof
fix xa :: 'a
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> xa \<in> cball x e \<Longrightarrow> (xa, 0::'c) \<in> H ` cball (x, y) r2"
using r2
 apply (auto)
using r2_def
by (force) 
...
mproof
fix xa :: 'a
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> xa \<in> cball x e \<Longrightarrow> cball (x, y) r2 \<subseteq> ball (x, y) r"
using r2
by (force simp:) 
mqed
mproof
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> continuous_on (cball x e) (\<lambda>xa. the_inv_into (cball (x, y) r2) H (xa, 0::'c))"
 apply (rule continuous_on_compose2[OF continuous_on_inv_into])
using r(2) r2(2)
 apply (auto simp: r2_def[symmetric] intro!: continuous_on_compose2[OF cH] continuous_intros)
 apply (rule inj_on_subset)
 apply (rule inj)
using r(2) r2(2)
 apply (force)
...
mproof
fix u :: 'a
assume prems: "\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v" "f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c)" "\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)" "dist x u \<le> e"
show "(u, snd (the_inv_into (ball (x, y) r) H (u, 0::'c))) \<in> S"
proof (-) 
have u: "u \<in> cball x e
...
define H' where H'_def: "H' \<equiv> \<lambda>x. (embed1_blinfun o\<^sub>L fst_blinfun) + (embed2_blinfun o\<^sub>L (f' x))"
have f'_inv: "f' (x, y) o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using T1 T2 apply (auto simp: invertibles_blinfun_def ac_simps intro!: exI[where x=T])
.
obtain d0 where e0: "0 < d0"  "ball (f' (x, y) o\<^sub>L embed2_blinfun) d0 \<subseteq> invertibles_blinfun"
using openE[OF blinfun_inverse_open f'_inv] apply (auto)
.
have fact0: "isCont (\<lambda>s. f' s o\<^sub>L embed2_blinfun) (x, y)"
 apply (auto intro!: continuous_intros f'C)
.
have fact1: "\<forall>\<^sub>F s in at (x, y). f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using fact0[unfolded isCont_def, THEN tendstoD, OF \<open>0 < d0\<close>] apply (eventually_elim)
using e0 apply (auto simp: subset_iff dist_commute) sorry

obtain e0 where fact2: "e0 > 0" "xa \<noteq> (x, y) \<Longrightarrow> dist xa (x, y) < e0 \<Longrightarrow>
        f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun" for xa
using fact1[unfolded "eventually_at"] apply (auto)
.
have e0: "e0 > 0" and "dist xa (x, y) < e0 \<Longrightarrow> f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun" for xa
mproof
show "0 < e0 \<Longrightarrow> (\<And>xa. xa \<noteq> (x, y) \<Longrightarrow> dist xa (x, y) < e0 \<Longrightarrow> f' xa o\<^sub>L embed2_blinfun \<in> invertibles_blinfun) \<Longrightarrow> 0 < e0"
 apply (simp)
.
mqed
using f'_inv apply (cases "xa = (x, y)")
 apply (auto)
.
have H': "x \<in> S \<Longrightarrow> (H has_derivative H' x) (at x)" for x
 apply (unfold "H_def" "H'_def")
 apply (auto intro!: derivative_eq_intros ext simp: blinfun.bilinear_simps)
.
have cH': "isCont H' (x, y)"
 apply (unfold "H'_def")
 apply (auto intro!: continuous_intros assms)
.
have linear_H': "\<And>s. s \<in> S \<Longrightarrow> linear (H' s)"
using H' assms(2) has_derivative_linear apply (blast)
.
have "*": "blinfun_apply T (blinfun_apply (f' (x, y)) (0, b)) = b" for b
using blinfun_inverseD[OF T2, of b] apply (simp)
.
have fact3: "inj (f' (x, y) o\<^sub>L embed2_blinfun)"
 apply (metis (no_types, lifting) "*" blinfun_apply_blinfun_compose embed2_blinfun.rep_eq injI)
.
have fact4[simp]: "blinfun_apply (f' (x, y)) (0, b) = 0 \<Longrightarrow> b = 0" for b
using fact3 apply (subst (asm) linear_injective_0)
mproof
show "blinfun_apply (f' (x, y)) (0::'a, b) = (0::'c) \<Longrightarrow> linear (blinfun_apply (f' (x, y) o\<^sub>L embed2_blinfun))"
 apply (rule bounded_linear.linear)
 apply (rule blinfun.bounded_linear_right)
...
define Hi where Hi_def: "Hi = (embed1_blinfun o\<^sub>L fst_blinfun) + ((embed2_blinfun o\<^sub>L T o\<^sub>L (snd_blinfun - (f' (x, y) o\<^sub>L embed1_blinfun o\<^sub>L fst_blinfun))))"
have Hi': "(\<lambda>u. snd (blinfun_apply Hi (u, 0))) = - T o\<^sub>L f' (x, y) o\<^sub>L embed1_blinfun"
 apply (auto simp: Hi_def blinfun.bilinear_simps)
.
have Hi: "Hi o\<^sub>L H' (x, y) = 1\<^sub>L"
 apply (auto simp: H'_def fun_eq_iff blinfun.bilinear_simps Hi_def intro!: ext blinfun_eqI)
 apply (subst blinfun_apply_Pair_split)
...
.
obtain r0 where r0: "0 < r0" and "ball (x, y) r0 \<subseteq> S" and inj: "inj_on H (ball (x, y) r0)"
using has_derivative_locally_injective_blinfun[OF S Hi H' cH'] apply (auto)
.
define r where r_def: "r = min r0 e0"
have r: "0 < r" and "ball (x, y) r \<subseteq> S" and inj: "inj_on H (ball (x, y) r)" and r_inv: "\<And>s. s \<in> ball (x, y) r \<Longrightarrow> f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
mproof
show "0 < r"
using e0 r0 apply (auto simp: r_def)
.
mqed
mproof
show "ball (x, y) r \<subseteq> S"
...
mproof
show "inj_on H (ball (x, y) r)"
using inj apply (rule inj_on_subset)
using e0 r0 apply (auto simp: r_def)
.
mqed
mproof
fix s :: "'a \<times> 'b"
show "s \<in> ball (x, y) r \<Longrightarrow> f' s o\<^sub>L embed2_blinfun \<in> invertibles_blinfun"
using e0 r0 apply (auto simp: r_def dist_commute)
.
mqed
.
obtain i :: 'a where fact6: "i \<in> Basis"
using nonempty_Basis apply (blast)
.
define undef where undef_def: "undef \<equiv> (x, y) + r *\<^sub>R (i, 0)"
have ud: "\<not> dist (x, y) undef < r"
using \<open>r > 0\<close> \<open>i \<in> Basis\<close> apply (auto simp: undef_def dist_norm)
.
define G where G_def: "G \<equiv> the_inv_into (ball (x, y) r) H"
{
fix u and v
...
have fact10: "(u, v) = H (G (u, v))"
 apply (unfold "G_def")
 apply (subst f_the_inv_into_f[where f=H])
 apply (auto)
.
have fact11: "\<dots> = H (G (u, v))"
...
.
have fact12: "\<dots> = (fst (G (u, v)), f (G (u, v)))"
 apply (auto simp: H_def split_beta')
.
have fact13: "u = fst (G (u, v))" and "v = f (G (u, v))"
using fact10 fact11 fact12 apply (simp_all)
.
have fact14: "f (u, snd (G(u, v))) = v" and "u = fst (G (u, v))"
using fact13 apply ((metis prod.collapse)+)
.
}
note fact7 = this
note uvs = fact7
...
have fact15: "f (x, snd (G (x, 0))) = 0"
 apply (rule uv)
 apply (metis (mono_tags, lifting) H_def assms(6) case_prod_beta' centre_in_ball fst_conv image_iff r(1) snd_conv)
.
have cH: "continuous_on S H"
 apply (rule has_derivative_continuous_on)
 apply (subst at_within_open)
...
.
have fact16: "(x, f (x, y)) \<in> H ` ball (x, y) (r/2)"
using \<open>r > 0\<close> apply (auto simp: H_def)
.
obtain e' where e': "e' > 0" and "cball (x, f (x, y)) e' \<subseteq> H ` ball (x, y) (r/2)"
using open_contains_cball[THEN iffD1, OF oH, rule_format, OF fact16] apply (auto)
.
have inv_subset: "the_inv_into (ball (x, y) r) H a = the_inv_into R H a" if fact17: "a \<in> H ` R" and "R \<subseteq> (ball (x, y) r)" for a and R
 apply (rule the_inv_into_f_eq[OF inj])
 apply (rule f_the_inv_into_f)
 apply (rule inj_on_subset[OF inj])
 apply (fact)
...
.
have GH: "G (H z) = z" if fact18: "dist (x, y) z < r" for z
 apply (auto simp: G_def the_inv_into_f_f inj fact18)
.
define e where e_def: "e = min (e' / 2) e0"
define r2 where r2_def: "r2 = r / 2"
have r2: "r2 > 0" and "r2 < r"
using \<open>r > 0\<close> apply (auto simp: r2_def)
.
have fact19: "e > 0"
using e' e0 apply (auto simp: e_def)
.
have fact20: "cball x e \<times> cball (f (x, y)) e \<subseteq> H ` ball (x, y) (r/2)"
using cball_times_subset[of "x" e' "f (x, y)"] e' apply (force simp: e_def)
.
have e_r_subset: "z \<in> cball x e \<Longrightarrow> (z, 0) \<in> H ` ball (x, y) (r/2)" for z
using fact20 \<open>0 < e\<close> assms(6) apply (auto simp: H_def subset_iff)
.
have u0: "(u, 0) \<in> H ` ball (x, y) r" if fact21: "u \<in> cball x e" for u
 apply (rule rev_subsetD)
 apply (rule e_r_subset)
...
.
note fact23 = e_r_subset
have G2: "f (x, snd (G (x, 0))) = 0" and "snd (G (x, 0)) = y" and "\<And>u. u \<in> cball x e \<Longrightarrow> f (u, snd (G (u, 0))) = 0" and "continuous_on (cball x e) (\<lambda>u. snd (G (u, 0)))" and "(\<lambda>t. (t, snd (G (t, 0)))) ` cball x e \<subseteq> S" and "e > 0" and "((\<lambda>u. snd (G (u, 0))) has_derivative (\<lambda>u. snd (Hi (u, 0)))) (at x)"
using uv fact15 fact23 apply (auto simp: G_def split_beta' intro!: continuous_intros continuous_on_compose2[OF cH])
mproof
assume prems: "\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v" and "f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c)" and "\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)"
show "snd (the_inv_into (ball (x, y) r) H (x, 0::'c)) = y"
mproof
have fact24: "the_inv_into (ball (x, y) r) H (x, 0) = (x, y)"
 apply (rule the_inv_into_f_eq)
 apply (fact)
 apply (auto simp: H_def assms \<open>r > 0\<close>)
.
show "?thesis"
using fact24 apply (auto)
.
mqed
.
mqed
...
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> xa \<in> cball x e \<Longrightarrow> (xa, 0::'c) \<in> H ` cball (x, y) r2"
using r2 apply (auto)
using r2_def apply (force)
.
mqed
mproof
fix xa :: 'a
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> xa \<in> cball x e \<Longrightarrow> cball (x, y) r2 \<subseteq> ball (x, y) r"
using r2 apply (force simp:)
.
mqed
mproof
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> continuous_on (cball x e) (\<lambda>xa. the_inv_into (cball (x, y) r2) H (xa, 0::'c))"
 apply (rule continuous_on_compose2[OF continuous_on_inv_into])
using r(2) r2(2) apply (auto simp: r2_def[symmetric] intro!: continuous_on_compose2[OF cH] continuous_intros)
 apply (rule inj_on_subset)
 apply (rule inj)
...
mproof
fix u :: 'a
assume prems: "\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v" and "f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c)" and "\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)" and "dist x u \<le> e"
show "(u, snd (the_inv_into (ball (x, y) r) H (u, 0::'c))) \<in> S"
mproof
have u: "u \<in> cball x e"
using prems apply (auto)
.
note fact26 = G_r[OF u]
have fact27: "ball (x, y) r \<subseteq> S"
using r apply (simp)
.
have fact28: "(G (u, 0)) \<in> S"
using fact26 fact27 apply (msorry)
.
show "?thesis"
 apply (unfold "G_def" [ symmetric ])
using fact28[unfolded "G_def" [ symmetric ]] uvs(2)[OF u0, OF u] apply (metis prod.collapse)
.
mqed
.
mqed
mproof
show "(\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v) \<Longrightarrow> f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c) \<Longrightarrow> (\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)) \<Longrightarrow> 0 < e"
using \<open>e > 0\<close> apply (simp)
.
mqed
mproof
assume prems: "\<And>u v. (u, v) \<in> H ` ball (x, y) r \<Longrightarrow> f (u, snd (the_inv_into (ball (x, y) r) H (u, v))) = v" and "f (x, snd (the_inv_into (ball (x, y) r) H (x, 0::'c))) = (0::'c)" and "\<And>z. dist x z \<le> e \<Longrightarrow> (z, 0::'c) \<in> H ` ball (x, y) (r / 2)"
show "((\<lambda>u. snd (the_inv_into (ball (x, y) r) H (u, 0::'c))) has_derivative (\<lambda>u. snd (blinfun_apply Hi (u, 0::'c)))) (at x)"
mproof
have fact30: "(x, y) \<in> cball (x, y) r2"
using r2 apply (auto)
.
have fact31: "H (x, y) \<in> interior (H ` cball (x, y) r2)"
 apply (rule interiorI[OF oH])
using r2 apply (auto simp: r2_def)
.
have fact32: "cball (x, y) r2 \<subseteq> S"
using r r2 apply (auto)
.
have fact33: "\<And>z. z \<in> cball (x, y) r2 \<Longrightarrow> G (H z) = z"
using r2 apply (auto intro!: GH)
.
have fact34: "(G has_derivative Hi) (at (H (x, y)))"
using fact30 fact31 fact32 fact33 apply (rule has_derivative_inverse[where g = G and f = H, OF compact_cball _ _ continuous_on_subset[OF cH] _ H' _ _])
(*4 subgoals*)
 apply ((use S blinfun.bounded_linear_right in auto)[1])
(*NEXT*)
 apply ((use S blinfun.bounded_linear_right in auto)[1])
(*NEXT*)
 apply ((use S blinfun.bounded_linear_right in auto)[1])
...
using Hi apply (transfer)
 apply (auto)
.
mqed
(*end 4 subgoals*)
.
have g': "(G has_derivative Hi) (at (x, 0))"
using fact34 apply (auto simp: H_def assms)
.
show "?thesis"
 apply (unfold "G_def" [ symmetric ] "H_def" [ symmetric ])
...
mqed
.
note fact37 = \<open>r > 0\<close>
define u where u_def: "u \<equiv> \<lambda>x. snd (G (x, 0))"
have local_unique: "u s = v s" if solves: "(\<And>s. s \<in> U \<Longrightarrow> f (s, v s) = 0)" and i: "v x = y" and v: "continuous_on U v" and s: "s \<in> U" and s': "(s, v s) \<in> ball (x, y) r" and U: "U \<subseteq> cball x e" for U and v and s
mproof
...
.
have fact38: "(s, snd (G (s, 0))) = (G (s, 0))"
using GH H_def s s' solves apply (fastforce)
.
have fact39: "\<dots> \<in> ball (x, y) r"
 apply (unfold "G_def")
 apply (rule the_inv_into_into)
 apply (fact)
 apply (rule u0)
...
.
have fact40: "(s, u s) \<in> ball (x, y) r"
 apply (unfold "u_def")
using fact38[unfolded "u_def"] fact39[unfolded "u_def"] apply (msorry)
.
show "u s = v s"
using inj_onD[OF inj H_eq s' fact40] apply (auto)
.
mqed
.
show "?thesis"
using G2[unfolded "u_def" "Hi'"] fact37[unfolded "u_def" "Hi'"] local_unique[unfolded "u_def" "Hi'"] apply (standard)
.
mqed
.
