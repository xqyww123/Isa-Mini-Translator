...
mproof
assume Nv1_inter_E2_empty: "N\<^bsub>\<V>1\<^esub> \<inter> E\<^bsub>ES2\<^esub> = {}" and Nv2_inter_E1_empty: "N\<^bsub>\<V>2\<^esub> \<inter> E\<^bsub>ES1\<^esub> = {}"
have fact0: "\<lbrakk> set \<tau> \<subseteq> E\<^bsub>(ES1 \<parallel> ES2)\<^esub>;
      set lambda \<subseteq> V\<^bsub>\<V>\<^esub>; 
      set t1 \<subseteq> E\<^bsub>ES1\<^esub>;
      set t2 \<subseteq> E\<^bsub>ES2\<^esub>;
      ((\<tau> \<upharpoonleft> E\<^bsub>ES1\<^esub>) @ t1) \<in> Tr\<^bsub>ES1\<^esub>;
      ...
      (t1 \<upharpoonleft> C\<^bsub>\<V>1\<^esub>) = [];
      (t2 \<upharpoonleft> C\<^bsub>\<V>2\<^esub>) = [] \<rbrakk>  
      \<Longrightarrow> (\<exists> t. ((\<tau> @ t) \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub> \<and> (t \<upharpoonleft> V\<^bsub>\<V>\<^esub>) = lambda \<and> (t \<upharpoonleft> C\<^bsub>\<V>\<^esub>) = []))" if "\<tau>_in_Estar": "set \<tau> \<subseteq> E\<^bsub>(ES1 \<parallel> ES2)\<^esub>" and lambda_in_Vvstar: "set lambda \<subseteq> V\<^bsub>\<V>\<^esub>" and t1_in_E1star: "set t1 \<subseteq> E\<^bsub>ES1\<^esub>" and t2_in_E2star: "set t2 \<subseteq> E\<^bsub>ES2\<^esub>" and "\<tau>_E1_t1_in_Tr1": "((\<tau> \<upharpoonleft> E\<^bsub>ES1\<^esub>) @ t1) \<in> Tr\<^bsub>ES1\<^esub>" and "\<tau>_E2_t2_in_Tr2": "((\<tau> \<upharpoonleft> E\<^bsub>ES2\<^esub>) @ t2) \<in> Tr\<^bsub>ES2\<^esub>" and lambda_E1_is_t1_Vv: "(lambda \<upharpoonleft> E\<^bsub>ES1\<^esub>) = (t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub>)" and lambda_E2_is_t2_Vv: "(lambda \<upharpoonleft> E\<^bsub>ES2\<^esub>) = (t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub>)" and t1_no_Cv1: "(t1 \<upharpoonleft> C\<^bsub>\<V>1\<^esub>) = []" and t2_no_Cv2: "(t2 \<upharpoonleft> C\<^bsub>\<V>2\<^esub>) = []" for \<tau> and lambda and t1 and t2
 apply ((induct lambda arbitrary: \<tau> t1 t2)[1])
(*2 subgoals*)
mproof
case Nil:   (Nil \<tau> t1 t2)
have fact2: "(\<tau> @ []) \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub>"
mproof
have fact3: "\<tau> \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub>"
mproof
have fact4: "\<tau> \<upharpoonleft> E\<^bsub>ES1\<^esub> \<in> Tr\<^bsub>ES1\<^esub>"
using Nil(5) validES1 apply ((simp add: ES_valid_def traces_prefixclosed_def prefixclosed_def prefix_def)[1])
.
have fact5: "\<tau> \<upharpoonleft> E\<^bsub>ES2\<^esub> \<in> Tr\<^bsub>ES2\<^esub>"
using Nil(6) validES2 apply ((simp add: ES_valid_def traces_prefixclosed_def prefixclosed_def prefix_def)[1])
.
note fact6 = Nil(1)
show "?thesis"
using fact4 fact5 fact6 apply ((simp add: composeES_def)[1])
.
mqed
.
show "?thesis"
...
mqed
.
have fact9: "([] \<upharpoonleft> V\<^bsub>\<V>\<^esub>) = []"
 apply ((simp add: projection_def)[1])
.
have fact10: "([] \<upharpoonleft> C\<^bsub>\<V>\<^esub>) = []"
 apply ((simp add: projection_def)[1])
.
show "?case"
using fact2 fact9 fact10 apply ((blast)[1])
.
mqed
(*NEXT*)
mproof
case Cons:   (Cons \<V>' lambda' \<tau> t1 t2)
have v'_in_Vv: "\<V>' \<in> V\<^bsub>\<V>\<^esub>"
using Cons(3) apply ((auto)[1])
.
have fact13: "\<V>' \<in> V\<^bsub>\<V>1\<^esub> \<inter> V\<^bsub>\<V>2\<^esub> 
              \<or> \<V>' \<in> V\<^bsub>\<V>1\<^esub> - E\<^bsub>ES2\<^esub>
              \<or> \<V>' \<in> V\<^bsub>\<V>2\<^esub> - E\<^bsub>ES1\<^esub>"
using Vv_is_Vv1_union_Vv2[unfolded "properSeparationOfViews_def"] v'_in_Vv[unfolded "properSeparationOfViews_def"] propSepViews[unfolded "properSeparationOfViews_def"] apply ((fastforce)[1])
.
have fact14: "?thesis" if v'_in_Vv1_inter_Vv2: "\<V>' \<in> V\<^bsub>\<V>1\<^esub> \<inter> V\<^bsub>\<V>2\<^esub>"
mproof
have v'_in_Vv1: "\<V>' \<in> V\<^bsub>\<V>1\<^esub>" and v'_in_Vv2: "\<V>' \<in> V\<^bsub>\<V>2\<^esub>"
(*2 subgoals*)
using v'_in_Vv1_inter_Vv2 apply ((auto)[1])
(*NEXT*)
using v'_in_Vv1_inter_Vv2 apply ((auto)[1])
(*end 2 subgoals*)
.
have v'_in_E1: "\<V>' \<in> E\<^bsub>ES1\<^esub>" and v'_in_E2: "\<V>' \<in> E\<^bsub>ES2\<^esub>"
(*2 subgoals*)
using v'_in_Vv[unfolded "properSeparationOfViews_def"] propSepViews[unfolded "properSeparationOfViews_def"] v'_in_Vv1[unfolded "properSeparationOfViews_def"] v'_in_Vv2[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
(*NEXT*)
using v'_in_Vv[unfolded "properSeparationOfViews_def"] propSepViews[unfolded "properSeparationOfViews_def"] v'_in_Vv1[unfolded "properSeparationOfViews_def"] v'_in_Vv2[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
(*end 2 subgoals*)
.
have fact15: "t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # (lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub>)"
using Cons(2,4,8) v'_in_E1 apply ((simp add: projection_def)[1])
.
obtain r1 and s1 where t1_is_r1_v'_s1: "t1 = r1 @ [\<V>'] @ s1" and r1_Vv_empty: "r1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = []"
using projection_split_first[OF fact15] apply ((auto)[1])
.
have r1_Vv1_empty: "r1 \<upharpoonleft> V\<^bsub>\<V>1\<^esub> = []"
using Vv_is_Vv1_union_Vv2 projection_on_subset[of "V\<^bsub>\<V>1\<^esub>" "V\<^bsub>\<V>\<^esub>" "r1"] t1_is_r1_v'_s1 r1_Vv_empty apply ((auto)[1])
.
have fact16: "t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # (lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub>)"
using Cons(3,5,9) v'_in_E2 apply ((simp add: projection_def)[1])
.
obtain r2 and s2 where t2_is_r2_v'_s2: "t2 = r2 @ [\<V>'] @ s2" and r2_Vv_empty: "r2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = []"
using projection_split_first[OF fact16] apply ((auto)[1])
.
have r2_Vv2_empty: "r2 \<upharpoonleft> V\<^bsub>\<V>2\<^esub> = []"
using Vv_is_Vv1_union_Vv2 projection_on_subset[of "V\<^bsub>\<V>2\<^esub>" "V\<^bsub>\<V>\<^esub>" "r2"] t2_is_r2_v'_s2 r2_Vv_empty apply ((auto)[1])
.
have r1_Cv1_empty: "r1 \<upharpoonleft> C\<^bsub>\<V>1\<^esub> = []"
using t1_is_r1_v'_s1 Cons(10) apply ((simp add: projection_concatenation_commute)[1])
.
have s1_Cv1_empty: "s1 \<upharpoonleft> C\<^bsub>\<V>1\<^esub> = []"
using t1_is_r1_v'_s1 Cons(10) apply ((simp only: projection_concatenation_commute)[1])
using t1_is_r1_v'_s1 Cons(10) apply ((auto)[1])
.
have r1_in_E1star: "set r1 \<subseteq> E\<^bsub>ES1\<^esub>" and s1_in_E1star: "set s1 \<subseteq> E\<^bsub>ES1\<^esub>"
(*2 subgoals*)
using Cons(4) t1_is_r1_v'_s1 apply ((auto)[1])
(*NEXT*)
using Cons(4) t1_is_r1_v'_s1 apply ((auto)[1])
(*end 2 subgoals*)
.
have "\<tau>E1_r1_v'_s1_in_Tr1": "\<tau> \<upharpoonleft> E\<^bsub>ES1\<^esub> @ r1 @ [\<V>'] @ s1 \<in> Tr\<^bsub>ES1\<^esub>"
using Cons(6) t1_is_r1_v'_s1 apply ((simp)[1])
.
have r1_in_Nv1star: "set r1 \<subseteq> N\<^bsub>\<V>1\<^esub>"
mproof
note fact17 = r1_in_E1star
have fact18: "set r1 \<inter> V\<^bsub>\<V>1\<^esub> = {}"
using r1_Vv1_empty apply ((metis Compl_Diff_eq Diff_cancel Diff_eq Int_commute Int_empty_right disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
have fact19: "set r1 \<inter> C\<^bsub>\<V>1\<^esub> = {}"
using r1_Cv1_empty apply ((metis Compl_Diff_eq Diff_cancel Diff_eq Int_commute Int_empty_right disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
note fact20 = validV1
show "?thesis"
using fact17 fact18 fact19 fact20 apply ((simp add: isViewOn_def V_valid_def VN_disjoint_def NC_disjoint_def)[1])
using fact17 fact18 fact19 fact20 apply ((auto)[1])
.
mqed
.
have r1E2_empty: "r1 \<upharpoonleft> E\<^bsub>ES2\<^esub> = []"
using Nv1_inter_E2_empty r1_in_Nv1star apply ((metis Int_commute empty_subsetI projection_on_subset2 r1_Vv_empty)[1])
.
have r2_Cv2_empty: "r2 \<upharpoonleft> C\<^bsub>\<V>2\<^esub> = []"
using t2_is_r2_v'_s2 Cons(11) apply ((simp add: projection_concatenation_commute)[1])
.
have s2_Cv2_empty: "s2 \<upharpoonleft> C\<^bsub>\<V>2\<^esub> = []"
using t2_is_r2_v'_s2 Cons(11) apply ((simp only: projection_concatenation_commute)[1])
using t2_is_r2_v'_s2 Cons(11) apply ((auto)[1])
.
have r2_in_E2star: "set r2 \<subseteq> E\<^bsub>ES2\<^esub>" and s2_in_E2star: "set s2 \<subseteq> E\<^bsub>ES2\<^esub>"
(*2 subgoals*)
using Cons(5) t2_is_r2_v'_s2 apply ((auto)[1])
(*NEXT*)
using Cons(5) t2_is_r2_v'_s2 apply ((auto)[1])
(*end 2 subgoals*)
.
have "\<tau>E2_r2_v'_s2_in_Tr2": "\<tau> \<upharpoonleft> E\<^bsub>ES2\<^esub> @ r2 @ [\<V>'] @ s2 \<in> Tr\<^bsub>ES2\<^esub>"
using Cons(7) t2_is_r2_v'_s2 apply ((simp)[1])
.
have r2_in_Nv2star: "set r2 \<subseteq> N\<^bsub>\<V>2\<^esub>"
mproof
note fact22 = r2_in_E2star
have fact23: "set r2 \<inter> V\<^bsub>\<V>2\<^esub> = {}"
using r2_Vv2_empty apply ((metis Compl_Diff_eq Diff_cancel Un_upper2 disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
have fact24: "set r2 \<inter> C\<^bsub>\<V>2\<^esub> = {}"
using r2_Cv2_empty apply ((metis Compl_Diff_eq Diff_cancel Un_upper2 disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
note fact25 = validV2
show "?thesis"
using fact22 fact23 fact24 fact25 apply ((simp add: isViewOn_def V_valid_def VN_disjoint_def NC_disjoint_def)[1])
using fact22 fact23 fact24 fact25 apply ((auto)[1])
.
mqed
.
have r2E1_empty: "r2 \<upharpoonleft> E\<^bsub>ES1\<^esub> = []"
using Nv2_inter_E1_empty r2_in_Nv2star apply ((metis Int_commute empty_subsetI projection_on_subset2 r2_Vv_empty)[1])
.
let ?tau = "\<tau> @ r1 @ r2 @ [\<V>']"
have fact27: "set ?tau \<subseteq> (E\<^bsub>(ES1 \<parallel> ES2)\<^esub>)"
using Cons(2) r1_in_E1star r2_in_E2star v'_in_E2 apply ((simp add: composeES_def)[1])
using Cons(2) r1_in_E1star r2_in_E2star v'_in_E2 apply ((auto)[1])
.
have fact28: "set lambda' \<subseteq> V\<^bsub>\<V>\<^esub>"
using Cons(3) apply ((auto)[1])
.
note fact29 = s1_in_E1star s2_in_E2star
have fact30: "((?tau \<upharpoonleft> E\<^bsub>ES1\<^esub>) @ s1) \<in> Tr\<^bsub>ES1\<^esub>"
using Cons(6) r1_in_E1star r2E1_empty v'_in_E1 t1_is_r1_v'_s1 apply ((simp only: projection_concatenation_commute list_subset_iff_projection_neutral projection_def)[1])
using Cons(6) r1_in_E1star r2E1_empty v'_in_E1 t1_is_r1_v'_s1 apply ((auto)[1])
.
have fact31: "((?tau \<upharpoonleft> E\<^bsub>ES2\<^esub>) @ s2) \<in> Tr\<^bsub>ES2\<^esub>"
using Cons(7) r2_in_E2star r1E2_empty v'_in_E2 t2_is_r2_v'_s2 apply ((simp only: projection_concatenation_commute list_subset_iff_projection_neutral projection_def)[1])
using Cons(7) r2_in_E2star r1E2_empty v'_in_E2 t2_is_r2_v'_s2 apply ((auto)[1])
.
have fact32: "lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub> = s1 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
mproof
have fact33: "t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = [\<V>'] @ (lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub>)"
using Cons(2,4,8) v'_in_E1 apply ((simp add: projection_def)[1])
.
have fact34: "t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = [\<V>'] @ (s1 \<upharpoonleft> V\<^bsub>\<V>\<^esub>)"
using t1_is_r1_v'_s1 r1_Vv_empty v'_in_Vv1 Vv_is_Vv1_union_Vv2 apply ((simp only: t1_is_r1_v'_s1 projection_concatenation_commute projection_def)[1])
using t1_is_r1_v'_s1 r1_Vv_empty v'_in_Vv1 Vv_is_Vv1_union_Vv2 apply ((auto)[1])
.
show "?thesis"
using fact33 fact34 apply ((auto)[1])
.
mqed
.
have fact36: "lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub> = s2 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
mproof
have fact37: "t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = [\<V>'] @ (lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub>)"
using Cons(3,5,9) v'_in_E2 apply ((simp add: projection_def)[1])
.
have fact38: "t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = [\<V>'] @ (s2 \<upharpoonleft> V\<^bsub>\<V>\<^esub>)"
using t2_is_r2_v'_s2 r2_Vv_empty v'_in_Vv2 Vv_is_Vv1_union_Vv2 apply ((simp only: t2_is_r2_v'_s2 projection_concatenation_commute projection_def)[1])
using t2_is_r2_v'_s2 r2_Vv_empty v'_in_Vv2 Vv_is_Vv1_union_Vv2 apply ((auto)[1])
.
show "?thesis"
using fact37 fact38 apply ((auto)[1])
.
mqed
.
note fact40 = s1_Cv1_empty s2_Cv2_empty "Cons.hyps"(1)[of ?tau s1 s2]
obtain t' where tau_t'_in_Tr: "?tau @ t' \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub>" and t'Vv_is_lambda': "t' \<upharpoonleft> V\<^bsub>\<V>\<^esub> = lambda'" and t'Cv_empty: "t' \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact27 fact28 fact29 fact30 fact31 fact32 fact36 fact40 apply ((auto)[1])
.
let ?t = "r1 @ r2 @ [\<V>'] @ t'"
note fact41 = tau_t'_in_Tr
have fact42: "?t \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # lambda'"
using r1_Vv_empty r2_Vv_empty t'Vv_is_lambda' v'_in_Vv apply ((simp add: projection_def)[1])
.
have fact43: "?t \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
mproof
have fact44: "C\<^bsub>\<V>\<^esub> \<inter> E\<^bsub>ES1\<^esub> \<subseteq> C\<^bsub>\<V>1\<^esub>"
using propSepViews[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have fact45: "r1 \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact44 apply ((metis projection_on_subset2 r1_Cv1_empty r1_in_E1star)[1])
.
have fact46: "C\<^bsub>\<V>\<^esub> \<inter> E\<^bsub>ES2\<^esub> \<subseteq> C\<^bsub>\<V>2\<^esub>"
using propSepViews[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have fact47: "r2 \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact46 apply ((metis projection_on_subset2 r2_Cv2_empty r2_in_E2star)[1])
.
note fact48 = v'_in_Vv VIsViewOnE t'Cv_empty
show "?thesis"
using fact45 fact47 fact48 apply ((simp add: isViewOn_def V_valid_def VC_disjoint_def projection_def)[1])
using fact45 fact47 fact48 apply ((auto)[1])
.
mqed
.
show fact14: "?thesis"
using fact41 fact42 fact43 apply ((auto)[1])
.
mqed
.
have fact51: "?thesis" if v'_in_Vv1_minus_E2: "\<V>' \<in> V\<^bsub>\<V>1\<^esub> - E\<^bsub>ES2\<^esub>"
mproof
have v'_in_Vv1: "\<V>' \<in> V\<^bsub>\<V>1\<^esub>"
using v'_in_Vv1_minus_E2 apply ((auto)[1])
.
have v'_in_E1: "\<V>' \<in> E\<^bsub>ES1\<^esub>"
using v'_in_Vv[unfolded "properSeparationOfViews_def"] propSepViews[unfolded "properSeparationOfViews_def"] v'_in_Vv1[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have v'_notin_E2: "\<V>' \<notin> E\<^bsub>ES2\<^esub>"
using v'_in_Vv1_minus_E2 apply ((auto)[1])
.
have v'_notin_Vv2: "\<V>' \<notin> V\<^bsub>\<V>2\<^esub>"
using validV2 v'_notin_E2 apply ((simp add: isViewOn_def V_valid_def)[1])
using validV2 v'_notin_E2 apply ((auto)[1])
.
have fact52: "t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # (lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub>)"
using Cons(3) Cons(4) Cons(8) v'_in_E1 apply ((simp add: projection_def)[1])
.
obtain r1 and s1 where t1_is_r1_v'_s1: "t1 = r1 @ [\<V>'] @ s1" and r1_Vv_empty: "r1 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = []"
using projection_split_first[OF fact52] apply ((auto)[1])
.
have r1_Vv1_empty: "r1 \<upharpoonleft> V\<^bsub>\<V>1\<^esub> = []"
using Vv_is_Vv1_union_Vv2 projection_on_subset[of "V\<^bsub>\<V>1\<^esub>" "V\<^bsub>\<V>\<^esub>" "r1"] t1_is_r1_v'_s1 r1_Vv_empty apply ((auto)[1])
.
...
have r1_in_Nv1star: "set r1 \<subseteq> N\<^bsub>\<V>1\<^esub>"
mproof
note fact53 = r1_in_E1star
have fact54: "set r1 \<inter> V\<^bsub>\<V>1\<^esub> = {}"
using r1_Vv1_empty apply ((metis Compl_Diff_eq Diff_cancel Diff_eq Int_commute Int_empty_right disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
have fact55: "set r1 \<inter> C\<^bsub>\<V>1\<^esub> = {}"
using r1_Cv1_empty apply ((metis Compl_Diff_eq Diff_cancel Diff_eq Int_commute Int_empty_right disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
note fact56 = validV1
show "?thesis"
using fact53 fact54 fact55 fact56 apply ((simp add: isViewOn_def V_valid_def VN_disjoint_def NC_disjoint_def)[1])
using fact53 fact54 fact55 fact56 apply ((auto)[1])
.
mqed
.
have r1E2_empty: "r1 \<upharpoonleft> E\<^bsub>ES2\<^esub> = []"
using Nv1_inter_E2_empty r1_in_Nv1star apply ((metis Int_commute empty_subsetI projection_on_subset2 r1_Vv1_empty)[1])
.
let ?tau = "\<tau> @ r1 @ [\<V>']"
have fact58: "set ?tau \<subseteq> E\<^bsub>(ES1 \<parallel> ES2)\<^esub>"
using v'_in_E1 Cons(2) r1_in_Nv1star validV1 apply ((simp only: isViewOn_def composeES_def V_valid_def)[1])
using v'_in_E1 Cons(2) r1_in_Nv1star validV1 apply ((auto)[1])
.
have fact59: "set lambda' \<subseteq> V\<^bsub>\<V>\<^esub>"
using Cons(3) apply ((auto)[1])
.
have fact60: "set s1 \<subseteq> E\<^bsub>ES1\<^esub>"
using Cons(4) t1_is_r1_v'_s1 apply ((auto)[1])
.
note fact61 = Cons(5)
have fact62: "?tau \<upharpoonleft> E\<^bsub>ES1\<^esub> @ s1 \<in> Tr\<^bsub>ES1\<^esub>"
 apply ((msorry)[1])
.
have fact63: "?tau \<upharpoonleft> E\<^bsub>ES2\<^esub> @ t2 \<in> Tr\<^bsub>ES2\<^esub>"
mproof
have fact64: "[\<V>'] \<upharpoonleft> E\<^bsub>ES2\<^esub> = []"
using v'_notin_E2 apply ((simp add: projection_def)[1])
.
show "?thesis"
using Cons(7) Cons(4) t1_is_r1_v'_s1 v'_notin_E2 r1_in_Nv1star Nv1_inter_E2_empty r1E2_empty fact64 apply ((simp only: t1_is_r1_v'_s1 list_subset_iff_projection_neutral projection_concatenation_commute)[1])
using Cons(7) Cons(4) t1_is_r1_v'_s1 v'_notin_E2 r1_in_Nv1star Nv1_inter_E2_empty r1E2_empty fact64 apply ((auto)[1])
.
mqed
.
have fact66: "lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub> = s1 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
using Cons(8) t1_is_r1_v'_s1 r1_Vv_empty v'_in_E1 v'_in_Vv apply ((simp add: projection_def)[1])
.
have fact67: "lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub> = t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
using Cons(9) v'_notin_E2 apply ((simp add: projection_def)[1])
.
note fact68 = s1_Cv1_empty Cons(11)
note fact69 = "Cons.hyps"(1)[of ?tau s1 t2]
obtain t' where tau_t'_in_Tr: "?tau @ t' \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub>" and t'_Vv_is_lambda': "t' \<upharpoonleft> V\<^bsub>\<V>\<^esub> = lambda'" and t'_Cv_empty: "t' \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact58 fact59 fact60 fact61 fact62 fact63 fact66 fact67 fact68 fact69 apply ((auto)[1])
.
let ?t = "r1 @ [\<V>'] @ t'"
note fact70 = tau_t'_in_Tr
have fact71: "?t \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # lambda'"
using r1_Vv_empty t'_Vv_is_lambda' v'_in_Vv apply ((simp add: projection_def)[1])
.
have fact72: "?t \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
mproof
have fact73: "C\<^bsub>\<V>\<^esub> \<inter> E\<^bsub>ES1\<^esub> \<subseteq> C\<^bsub>\<V>1\<^esub>"
using propSepViews[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have fact74: "r1 \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact73 apply ((metis projection_on_subset2 r1_Cv1_empty r1_in_E1star)[1])
.
show "?thesis"
using v'_in_Vv VIsViewOnE t'_Cv_empty fact74 apply ((simp add: isViewOn_def V_valid_def VC_disjoint_def projection_def)[1])
using v'_in_Vv VIsViewOnE t'_Cv_empty fact74 apply ((auto)[1])
.
mqed
.
show fact51: "?thesis"
using fact70 fact71 fact72 apply ((auto)[1])
.
mqed
.
have fact77: "?thesis" if v'_in_Vv2_minus_E1: "\<V>' \<in> V\<^bsub>\<V>2\<^esub> - E\<^bsub>ES1\<^esub>"
mproof
have v'_in_Vv2: "\<V>' \<in> V\<^bsub>\<V>2\<^esub>"
using v'_in_Vv2_minus_E1 apply ((auto)[1])
.
have v'_in_E2: "\<V>' \<in> E\<^bsub>ES2\<^esub>"
using v'_in_Vv[unfolded "properSeparationOfViews_def"] propSepViews[unfolded "properSeparationOfViews_def"] v'_in_Vv2[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have v'_notin_E1: "\<V>' \<notin> E\<^bsub>ES1\<^esub>"
using v'_in_Vv2_minus_E1 apply ((auto)[1])
.
have v'_notin_Vv1: "\<V>' \<notin> V\<^bsub>\<V>1\<^esub>"
using validV1 v'_notin_E1 apply ((simp add:isViewOn_def V_valid_def)[1])
using validV1 v'_notin_E1 apply ((auto)[1])
.
have fact78: "t2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # (lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub>)"
using Cons(4) Cons(5) Cons(9) v'_in_E2 apply ((simp add: projection_def)[1])
.
obtain r2 and s2 where t2_is_r2_v'_s2: "t2 = r2 @ [\<V>'] @ s2" and r2_Vv_empty: "r2 \<upharpoonleft> V\<^bsub>\<V>\<^esub> = []"
using projection_split_first[OF fact78] apply ((auto)[1])
.
have r2_Vv2_empty: "r2 \<upharpoonleft> V\<^bsub>\<V>2\<^esub> = []"
using Vv_is_Vv1_union_Vv2 projection_on_subset[of "V\<^bsub>\<V>2\<^esub>" "V\<^bsub>\<V>\<^esub>" "r2"] t2_is_r2_v'_s2 r2_Vv_empty apply ((auto)[1])
.
...
using t2_is_r2_v'_s2 Cons(11) apply ((auto)[1])
.
have r2_in_E2star: "set r2 \<subseteq> E\<^bsub>ES2\<^esub>"
using Cons(5) t2_is_r2_v'_s2 apply ((auto)[1])
.
have r2_in_Nv2star: "set r2 \<subseteq> N\<^bsub>\<V>2\<^esub>"
mproof
note fact79 = r2_in_E2star
have fact80: "set r2 \<inter> V\<^bsub>\<V>2\<^esub> = {}"
using r2_Vv2_empty apply ((metis Compl_Diff_eq Diff_cancel Un_upper2 disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
have fact81: "set r2 \<inter> C\<^bsub>\<V>2\<^esub> = {}"
using r2_Cv2_empty apply ((metis Compl_Diff_eq Diff_cancel Un_upper2 disjoint_eq_subset_Compl list_subset_iff_projection_neutral projection_on_union)[1])
.
note fact82 = validV2
show "?thesis"
using fact79 fact80 fact81 fact82 apply ((simp add: isViewOn_def V_valid_def VN_disjoint_def NC_disjoint_def)[1])
using fact79 fact80 fact81 fact82 apply ((auto)[1])
.
mqed
.
have r2E1_empty: "r2 \<upharpoonleft> E\<^bsub>ES1\<^esub> = []"
using Nv2_inter_E1_empty r2_in_Nv2star apply ((metis Int_commute empty_subsetI projection_on_subset2 r2_Vv2_empty)[1])
.
let ?tau = "\<tau> @ r2 @ [\<V>']"
have fact84: "set ?tau \<subseteq> E\<^bsub>(ES1 \<parallel> ES2)\<^esub>"
using v'_in_E2 Cons(2) r2_in_Nv2star validV2 apply ((simp only: composeES_def isViewOn_def V_valid_def)[1])
using v'_in_E2 Cons(2) r2_in_Nv2star validV2 apply ((auto)[1])
.
have fact85: "set lambda' \<subseteq> V\<^bsub>\<V>\<^esub>"
using Cons(3) apply ((auto)[1])
.
note fact86 = Cons(4)
have fact87: "set s2 \<subseteq> E\<^bsub>ES2\<^esub>"
using Cons(5) t2_is_r2_v'_s2 apply ((auto)[1])
.
have fact88: "?tau \<upharpoonleft> E\<^bsub>ES1\<^esub> @ t1 \<in> Tr\<^bsub>ES1\<^esub>"
mproof
have fact89: "[\<V>'] \<upharpoonleft> E\<^bsub>ES1\<^esub> = []"
using v'_notin_E1 apply ((simp add: projection_def)[1])
.
show "?thesis"
using Cons(6) Cons(3) t2_is_r2_v'_s2 v'_notin_E1 r2_in_Nv2star Nv2_inter_E1_empty r2E1_empty fact89 apply ((simp only: t2_is_r2_v'_s2 list_subset_iff_projection_neutral projection_concatenation_commute)[1])
using Cons(6) Cons(3) t2_is_r2_v'_s2 v'_notin_E1 r2_in_Nv2star Nv2_inter_E1_empty r2E1_empty fact89 apply ((auto)[1])
.
mqed
.
have fact91: "?tau \<upharpoonleft> E\<^bsub>ES2\<^esub> @ s2 \<in> Tr\<^bsub>ES2\<^esub>"
 apply ((msorry)[1])
.
have fact92: "lambda' \<upharpoonleft> E\<^bsub>ES1\<^esub> = t1 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
using Cons(8) v'_notin_E1 apply ((simp add: projection_def)[1])
.
have fact93: "lambda' \<upharpoonleft> E\<^bsub>ES2\<^esub> = s2 \<upharpoonleft> V\<^bsub>\<V>\<^esub>"
using Cons(9) t2_is_r2_v'_s2 r2_Vv_empty v'_in_E2 v'_in_Vv apply ((simp add: projection_def)[1])
.
note fact94 = Cons(10) s2_Cv2_empty
note fact95 = "Cons.hyps"(1)[of ?tau t1 s2]
obtain t' where tau_t'_in_Tr: "?tau @ t' \<in> Tr\<^bsub>(ES1 \<parallel> ES2)\<^esub>" and t'_Vv_is_lambda': "t' \<upharpoonleft> V\<^bsub>\<V>\<^esub> = lambda'" and t'_Cv_empty: "t' \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact84 fact85 fact86 fact87 fact88 fact91 fact92 fact93 fact94 fact95 apply ((auto)[1])
.
let ?t = "r2 @ [\<V>'] @ t'"
note fact96 = tau_t'_in_Tr
have fact97: "?t \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # lambda'"
using r2_Vv_empty t'_Vv_is_lambda' v'_in_Vv apply ((simp add: projection_def)[1])
.
have fact98: "?t \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
mproof
have fact99: "C\<^bsub>\<V>\<^esub> \<inter> E\<^bsub>ES2\<^esub> \<subseteq> C\<^bsub>\<V>2\<^esub>"
using propSepViews[unfolded "properSeparationOfViews_def"] apply ((auto)[1])
.
have fact100: "r2 \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact99 apply ((metis projection_on_subset2 r2_Cv2_empty r2_in_E2star)[1])
.
show "?thesis"
using v'_in_Vv VIsViewOnE t'_Cv_empty fact100 apply ((simp add: isViewOn_def V_valid_def VC_disjoint_def projection_def)[1])
using v'_in_Vv VIsViewOnE t'_Cv_empty fact100 apply ((auto)[1])
.
mqed
.
show fact77: "?thesis"
using fact96 fact97 fact98 apply ((auto)[1])
.
mqed
.
show "\<exists>t. \<tau> @ t \<in> Tr\<^bsub>ES1 \<parallel> ES2\<^esub> \<and> t \<upharpoonleft> V\<^bsub>\<V>\<^esub> = \<V>' # lambda' \<and> t \<upharpoonleft> C\<^bsub>\<V>\<^esub> = []"
using fact13 fact14 fact51 fact77 apply ((blast)[1])
.
mqed
(*end 2 subgoals*)
.
show "?thesis"
using fact0 apply ((auto)[1])
.
mqed
.
