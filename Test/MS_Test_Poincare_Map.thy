...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
(File.read (Path.explode "./Translator/t13.txt"))\<close>
...
using e0 apply (auto simp: subset_iff dist_commute)
.
...
have fact7: "f (u, snd (G(u, v))) = v" "u = fst (G (u, v))" if fact8[simp]: "(u, v) \<in> H ` ball (x, y) r" for u and v
mproof
note fact9[simp] = inj
...
.
show fact14: "f (u, snd (G(u, v))) = v" "u = fst (G (u, v))"
...
have fact14: "f (u, snd (G(u, v))) = v" "u = fst (G (u, v))" if [simp]: "(u, v) \<in> H ` ball (x, y) r" for u v
mproof
note fact9[simp] = inj
have fact10: "(u, v) = H (G (u, v))"
 apply (unfold "G_def")
