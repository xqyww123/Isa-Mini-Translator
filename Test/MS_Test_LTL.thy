...
show "\<xi> \<Turnstile>\<^sub>c (\<phi> M\<^sub>c \<psi>) or\<^sub>c (G\<^sub>c \<psi>)"
 apply ((cases "\<xi> \<Turnstile>\<^sub>c G\<^sub>c \<psi>")[1])
(*2 subgoals*)
 apply ((simp)[1])
(*NEXT*)
mproof
case False:  False
obtain i where fact1: "\<not> suffix i \<xi> \<Turnstile>\<^sub>c \<psi>" and fact2: "\<forall>j<i. suffix j \<xi> \<Turnstile>\<^sub>c \<psi>"
using False exists_least_iff[of "\<lambda>i. \<not> suffix i \<xi> \<Turnstile>\<^sub>c \<psi>"] apply ((force)[1])
.
show "\<xi> \<Turnstile>\<^sub>c \<phi> M\<^sub>c \<psi> or\<^sub>c G\<^sub>c \<psi>"
using fact1 fact2 asm apply ((force)[1])
.
mqed
(*end 2 subgoals*)
.
mqed
(*NEXT*)
 apply ((unfold semantics_ltlc.simps)[1])
 apply ((insert not_less)[1])
 apply ((blast)[1])
(*end 2 subgoals*)
(*NEXT*)
 apply ((safe)[1])
...
show "\<xi> \<Turnstile>\<^sub>c \<phi> M\<^sub>c \<psi>"
using asm apply ((cases "\<xi> \<Turnstile>\<^sub>c F\<^sub>c \<phi>")[1])
(*2 subgoals*)
mproof
case True:  True
obtain i where fact5: "suffix i \<xi> \<Turnstile>\<^sub>c \<phi>" and fact6: "\<forall>j<i. \<not> suffix j \<xi> \<Turnstile>\<^sub>c \<phi>"
using True exists_least_iff[of "\<lambda>i. suffix i \<xi> \<Turnstile>\<^sub>c \<phi>"] apply ((force)[1])
.
show "\<xi> \<Turnstile>\<^sub>c \<phi> M\<^sub>c \<psi>"
using fact5 fact6 asm apply ((force)[1])
.
mqed
(*NEXT*)
 apply ((simp)[1])
