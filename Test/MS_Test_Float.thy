...
by (min_script \<open>
APPLY (unfold "float_def")
APPLY (safe)
APPLY (simp)
INTRO VARS m1 m2 e1 e2
HAVE "*":"\<exists>(m::int) (e::int). m1 * 2 powr e1 + m2 * 2 powr e2 = m * 2 powr e" if "e1 \<le> e2" for e1 :: "int" and m1 :: "int" and e2 :: "int" and m2 :: "int"
  HAVE "fact0":"m1 * 2 powr e1 + m2 * 2 powr e2 = (m1 + m2 * 2 ^ nat (e2 - e1)) * 2 powr e1"
    APPLY (simp add : powr_diff field_simps flip : powr_realpow) WITH that
  END 
  APPLY (blast) WITH fact0
END 
CONSIDER "1":"e2 \<le> e1" | "2":"e1 \<le> e2"
  APPLY (rule linorder_le_cases)
  (*generates 2 subgoals*)
  NEXT 
NEXT 
(*generates 2 subgoals*)
  APPLY (simp add : ac_simps) WITH *[OF 1, of m2 m1]
NEXT 
  APPLY (rule *) WITH 2
END 
\<close>)
...
(*
 apply ((unfold "float_def")[1])
 apply ((safe)[1])
 apply ((simp)[1])
mproof
fix e1 :: "int" and m1 :: "int" and e2 :: "int" and m2 :: "int"
have "*":"\<exists>(m::int) (e::int). m1 * 2 powr e1 + m2 * 2 powr e2 = m * 2 powr e" if "e1 \<le> e2" for e1 :: "int" and m1 :: "int" and e2 :: "int" and m2 :: "int"
mproof
have "fact0":"m1 * 2 powr e1 + m2 * 2 powr e2 = (m1 + m2 * 2 ^ nat (e2 - e1)) * 2 powr e1"
using that apply ((simp add : powr_diff field_simps flip : powr_realpow)[1])
.
show "?thesis"
using fact0 apply ((blast)[1])
.
mqed
.
consider "e2 \<le> e1" | "e1 \<le> e2"
 apply ((rule linorder_le_cases)[1])
(*2 subgoals*)
 apply ((assumption)[1])
(*NEXT*)
.
...
using fact1 apply ((cases)[1])
(*2 subgoals*)
mproof
case "1": 1
show "?thesis"
  using *[OF "1", of m2 m1] apply (simp add: ac_simps)
 apply ((msorry)[1])
.
mqed
(*NEXT*)
