...
theory MS_Test_Parity
  imports HOL.Euclidean_Rings Minilang_Translator.MS_Translator
begin
...
qed
  term 1
  ML_ val \<open>val _ =  MinLang_Translator.elaborate_tatic'test (SOME "type")  @{Isar.state} 
    (File.read (Path.explode "/tmp/t4.txt"))\<close>

proof (cases \<open>m < n\<close>)
  (*goals:
    1. \<open>(m::num) < (n::num) \<Longrightarrow> MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m n = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>
    2. \<open>\<not> (m::num) < (n::num) \<Longrightarrow> MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m n = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>*)
  case True (*\<open>(m::num) < (n::num)\<close>*)
  then show "?thesis"
    (*goal: \<open>MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod (m::num) (n::num) = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>*)
    by (msorry)
next
  (*goal: \<open>\<not> (m::num) < (n::num) \<Longrightarrow> MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m n = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>*)
  case False (*\<open>\<not> (m::num) < (n::num)\<close>*)
  define r and s and t where "r = (numeral m :: nat)" "s = (numeral n :: nat)" "t = 2 * s"
  then have "*": "numeral m = of_nat r" "numeral n = of_nat s" "numeral (num.Bit0 n) = of_nat t" and "\<not> s \<le> r mod s"
    (*goals:
      1. \<open>numeral (m::num) = of_nat (r::nat)\<close>
      2. \<open>numeral (n::num) = of_nat (s::nat)\<close>
      3. \<open>numeral (num.Bit0 (n::num)) = of_nat (t::nat)\<close>
      4. \<open>\<not> (s::nat) \<le> (r::nat) mod s\<close>
    discuss goal 1*)
      apply ((msorry)[1])
    (*discuss goal 2*)
      apply ((msorry)[1])
    ...
    (*discuss goal 4*)
      apply ((msorry)[1])
    (*proven 4 subgoals*) .
  have t: "2 * (r div t) = r div s - r div s mod 2" "r mod t = s * (r div s mod 2) + r mod s"
    (*goals:
      1. \<open>(2::nat) * ((r::nat) div (t::nat)) = r div (s::nat) - r div s mod (2::nat)\<close>
      2. \<open>(r::nat) mod (t::nat) = (s::nat) * (r div s mod (2::nat)) + r mod s\<close>
    discuss goal 1*)
      apply (simp add: Rings.minus_mod_eq_mult_div (*\<open>(?a::?'a) - ?a mod (?b::?'a) = ?b * (?a div ?b)\<close>*) Groups.mult.commute [of 2] (*\<open>(2::?'b1) * (?b::?'b1) = ?b * (2::?'b1)\<close>*) Euclidean_Rings.div_mult2_eq (*\<open>(?m::nat) div ((?n::nat) * (?q::nat)) = ?m div ?n div ?q\<close>*) \<open>t = 2 * s\<close>)
    (*discuss goal 2*)
      apply (use mod_mult2_eq [of r s 2] in \<open>simp add: ac_simps \<open>t = 2 * s\<close>\<close>)
    (*proven 2 subgoals*) .
  have rs: "r div s mod 2 = 0 \<or> r div s mod 2 = Suc 0"
    by auto
  from \<open>\<not> s \<le> r mod s\<close> (*\<open>\<not> (s::nat) \<le> (r::nat) mod s\<close>*) have "s \<le> r mod t \<Longrightarrow>
     r div s = Suc (2 * (r div t)) \<and>
     r mod s = r mod t - s"
    using rs (*\<open>(r::nat) div (s::nat) mod (2::nat) = (0::nat) \<or> r div s mod (2::nat) = Suc (0::nat)\<close>*) by (auto simp add: t (*\<open>(2::nat) * ((r::nat) div (t::nat)) = r div (s::nat) - r div s mod (2::nat)\<close> \<open>(r::nat) mod (t::nat) = (s::nat) * (r div s mod (2::nat)) + r mod s\<close>*))
  moreover have "r mod t < s \<Longrightarrow>
     r div s = 2 * (r div t) \<and>
     r mod s = r mod t"
    using rs (*\<open>(r::nat) div (s::nat) mod (2::nat) = (0::nat) \<or> r div s mod (2::nat) = Suc (0::nat)\<close>*) by (auto simp add: t (*\<open>(2::nat) * ((r::nat) div (t::nat)) = r div (s::nat) - r div s mod (2::nat)\<close> \<open>(r::nat) mod (t::nat) = (s::nat) * (r div s mod (2::nat)) + r mod s\<close>*))
  ultimately show "?thesis"
    (*goal: \<open>MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod (m::num) (n::num) = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>*)
    apply (simp add: divmod_def (*\<open>MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod (?m::num) (?n::num) = (numeral ?m div numeral ?n, numeral ?m mod numeral ?n)\<close>*) prod_eq_iff (*\<open>((?s::?'a \<times> ?'b) = (?t::?'a \<times> ?'b)) = (fst ?s = fst ?t \<and> snd ?s = snd ?t)\<close>*) split_def (*\<open>case_prod = (\<lambda>(c::?'a \<Rightarrow> ?'b \<Rightarrow> ?'c) p::?'a \<times> ?'b. c (fst p) (snd p))\<close>*) Let_def (*\<open>Let (?s::?'a) (?f::?'a \<Rightarrow> ?'b) \<equiv> ?f ?s\<close>*) not_less (*\<open>(\<not> (?x::'a) < (?y::'a)) = (?y \<le> ?x)\<close>*) mod_eq_0_iff_dvd (*\<open>((?a::'a) mod (?b::'a) = (0::'a)) = (?b dvd ?a)\<close>*) Rings.mod_eq_0_iff_dvd (*\<open>((?a::?'a) mod (?b::?'a) = (0::?'a)) = (?b dvd ?a)\<close>*) False (*\<open>\<not> (m::num) < (n::num)\<close>*) not_le (*\<open>(\<not> (?x::'a) \<le> (?y::'a)) = (?y < ?x)\<close>*) * (*\<open>numeral (m::num) = of_nat (r::nat)\<close> \<open>numeral (n::num) = of_nat (s::nat)\<close> \<open>numeral (num.Bit0 (n::num)) = of_nat (t::nat)\<close>*))
    (*goal: \<open>MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod (m::num) (n::num) = (if m < n then (0::'a, numeral m) else MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod_step (numeral n) (MS_Test_Parity.linordered_euclidean_semiring_division_class.divmod m (num.Bit0 n)))\<close>*)
    by (simp add: flip: of_nat_numeral (*\<open>of_nat (numeral (?n::num)) = numeral ?n\<close>*) of_nat_mult (*\<open>of_nat ((?m::nat) * (?n::nat)) = of_nat ?m * of_nat ?n\<close>*) add.commute [of 1] (*\<open>(1::'a) + (?b::'a) = ?b + (1::'a)\<close>*) of_nat_div (*\<open>of_nat ((?m::nat) div (?n::nat)) = of_nat ?m div of_nat ?n\<close>*) of_nat_mod (*\<open>of_nat ((?m::nat) mod (?n::nat)) = of_nat ?m mod of_nat ?n\<close>*) of_nat_Suc (*\<open>of_nat (Suc (?m::nat)) = (1::'a) + of_nat ?m\<close>*) of_nat_diff (*\<open>(?n::nat) \<le> (?m::nat) \<Longrightarrow> of_nat (?m - ?n) = of_nat ?m - of_nat ?n\<close>*))
qed
