...
theory MS_Test_Sequence_Zip
imports Transition_Systems_and_Automata.Sequence_LTL Minilang_Translator.MS_Translator
begin
...
RULE 
(*goal: "\<And>y z. \<lbrakk>\<And>x. PROP P x; length y = length z\<rbrakk> \<Longrightarrow> PROP P (y || z)" and "\<And>x. (\<And>y z. length y = length z \<Longrightarrow> PROP P (y || z)) \<Longrightarrow> PROP P x" *)
NEXT
  (*goal: "\<And>x. (\<And>y z. length y = length z \<Longrightarrow> PROP P (y || z)) \<Longrightarrow> PROP P x" *)
END WITH zip_map_fst_snd
