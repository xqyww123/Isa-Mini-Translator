...
using S apply ((induct n)[1])
(*2 subgoals*)
mproof
case 0:  0
show "?case"
using "0" apply ((simp)[1])
.
mqed
(*NEXT*)
mproof
case Suc:   (Suc n)
have fact1: "n \<le> enumerate S n"
using Suc apply ((simp)[1])
.
note fact2 = enumerate_mono[of n "Suc n", OF _ \<open>infinite S\<close>]
show "?case"
using fact1 fact2 apply ((simp)[1])
.
mqed
(*end 2 subgoals*)
.
...
INDUCT n WITH S
  APPLY (simp) WITH case_0
NEXT VARS n
  HAVE fact4: "n < Suc n \<Longrightarrow> n < wellorder_class.enumerate S (Suc n)"
    STEP "n \<le> wellorder_class.enumerate S n"
    NEXT WITH Suc
    STEP "\<dots> < wellorder_class.enumerate S (Suc n)"
    NEXT WITH enumerate_mono[of n "Suc n", OF _ \<open>infinite S\<close>]
...
\proof clarsimp\n\
  \fix y\n\
  \show \"\<And>x. enumerate S x = enumerate S y \<Longrightarrow> x = y\"\n\
    \proof -\n\
    \fix x\n\
    \show \"enumerate S x = enumerate S y \<Longrightarrow> x = y\"\n\
...
show "\<And>x y. enumerate S x = enumerate S y \<Longrightarrow> x = y"
 apply ((metis neq_iff enumerate_mono[OF _ \<open>infinite S\<close>])[1])
.
mqed
.
...
mproof
show "bij_betw (enumerate M) {..<card M} M"
apply (simp add : assms finite_bij_enumerate)
.
mqed
(*NEXT*)
mproof
show "strict_mono_on {..<card M} (enumerate M)"
apply (simp add : assms finite_enumerate_mono strict_mono_on_def)
.
mqed
(*end 2 subgoals*)
.
