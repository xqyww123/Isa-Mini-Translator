...
imports
  ...
  HOL.Power
  HOL.Complex
  Jordan_Normal_Form.Jordan_Normal_Form Minilang_Translator.MS_Translator
begin
...
next
  ...
  then have "1\<^sub>m (2^m) $$ (i div 2^n, j div 2^n) = (0::'a) \<or> 1\<^sub>m (2^n) $$ (i mod 2^n, j mod 2^n) = 0"
    using assms by simp
  then show ?thesis
