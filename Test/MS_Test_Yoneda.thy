...
theory MS_Test_Yoneda
imports Category3.DualCategory Category3.SetCat Category3.FunctorCategory MS_Translator
begin
...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
      "using assms \<psi>_def set_def local_inj by auto"\<close>
...
ML \<open>Multithreading.max_threads ()\<close>
...
\<close>   
    ...
    ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
(File.read (Path.explode "./Translator/t6.txt"))\<close>
...
have arr: "S.arr (map gf)"
using gf arr_map apply ((blast)[1])
.
show "S.arr (map gf)"
using gf arr_map apply ((blast)[1])
.
...
using arr gf "local.map_def" map_ide apply ((auto)[1])
.
mqed
(*NEXT*)
mproof
...
using gf set_subset_Univ "\<psi>_mapsto" map_def set_def "S.arr_mkIde" arr map_ide apply ((auto)[1])
.
mqed
(*NEXT*)
mproof
...
have seq: "C.arr (fst gf) \<and> C.arr (snd gf) \<and> C.dom (snd gf') = C.cod (snd gf) \<and>
                  C.arr (fst gf') \<and> C.arr (snd gf') \<and> C.dom (fst gf) = C.cod (fst gf')"
using gf' apply ((elim CopxC.seqE C.seqE)[1])
using gf' apply ((auto)[1])
.
...
have 1: "map (gf' \<odot> gf) =
                    S.mkArr (set (CopxC.dom gf)) (set (CopxC.cod gf'))
                            (\<phi> (CopxC.cod gf') o (\<lambda>h. snd (gf' \<odot> gf) \<cdot> h \<cdot> fst (gf' \<odot> gf))
                                               o \<psi> (CopxC.dom gf))"
using gf' map_def "CopxC.cod_comp" "CopxC.dom_comp" apply ((auto)[1])
.
have fact3: "S.mkArr (local.set (CopxC.dom gf)) (local.set (CopxC.cod gf')) (\<phi> (CopxC.cod gf') \<circ> (\<lambda>h. snd (gf' \<odot> gf) \<cdot> h \<cdot> fst (gf' \<odot> gf)) \<circ> \<psi> (CopxC.dom gf)) = S.mkArr (local.set (CopxC.dom gf)) (local.set (CopxC.cod gf')) (\<phi> (CopxC.cod gf') \<circ> (\<lambda>h. snd gf' \<cdot> h \<cdot> fst gf') \<circ> \<psi> (CopxC.dom gf') \<circ> (\<phi> (CopxC.cod gf) \<circ> (\<lambda>h. snd gf \<cdot> h \<cdot> fst gf) \<circ> \<psi> (CopxC.dom gf)))"
 apply ((intro S.mkArr_eqI')[1])
(*2 subgoals*)
using "0" "1" apply ((simp)[1])
(*NEXT*)
using gf' "\<psi>_mapsto" set_def "\<psi>_\<phi>" "C.comp_assoc" apply ((fastforce)[1])
(*end 2 subgoals*)
.
have fact6: "S.mkArr (local.set (CopxC.dom gf)) (local.set (CopxC.cod gf')) (\<phi> (CopxC.cod gf') \<circ> (\<lambda>h. snd gf' \<cdot> h \<cdot> fst gf') \<circ> \<psi> (CopxC.dom gf') \<circ> (\<phi> (CopxC.cod gf) \<circ> (\<lambda>h. snd gf \<cdot> h \<cdot> fst gf) \<circ> \<psi> (CopxC.dom gf))) = local.map gf' \<cdot>\<^sub>S local.map gf"
using seq gf' map_def arr_map[of gf] arr_map[of gf'] "S.comp_mkArr" apply ((auto)[1])
.
show "map (gf' \<odot> gf) = map gf' \<cdot>\<^sub>S map gf"
using "1" fact3 fact6 seq gf' apply ((auto)[1])
.
mqed
(*end 5 subgoals*)
.
...
interpretation S: replete_setcat \<open>TYPE('a)\<close>  sorry
  
    lemma has_hom_functor:
      shows "hom_functor C S.comp S.setp (\<lambda>_. S.UP)"
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
      "using S.UP_mapsto S.inj_UP injD inj_onI\n\
      \by unfold_locales (auto simp add: inj_def inj_onI)"\<close>
...
(File.read (Path.explode "./Translator/t4.txt"))\<close>
 ...
 apply ((standard)[1])
(*2 subgoals*)
using ide_a e "\<T>\<^sub>o_in_hom" apply ((simp)[1])
(*NEXT*)
mproof
fix g :: 'c
assume g: "Cop.arr g"
...
mproof
have fact2: "S.arr (S.mkArr (Hom.set (?b', a)) (F.SET ?b') (\<lambda>s. F.FUN (\<psi> (?b', a) s) e)) \<and>
                S.dom (S.mkArr (Hom.set (?b', a)) (F.SET ?b') (\<lambda>s. F.FUN (\<psi> (?b', a) s) e))
                   = Y a ?b' \<and>
                S.cod (S.mkArr (Hom.set (?b', a)) (F.SET ?b') (\<lambda>s. F.FUN (\<psi> (?b', a) s) e))
                   = F ?b'"
using "Cop.cod_char" "\<T>\<^sub>o_def" "\<T>\<^sub>o_in_hom" e g apply ((metis Cop.ide_char Cop.ide_cod S.in_homE)[1])
.
have fact3: "Y a g = S.mkArr (Hom.set (?b, a)) (Hom.set (?b', a))
                                         (\<phi> (?b', a) \<circ> Cop.comp g \<circ> \<psi> (?b, a))"
using Y_ide_arr[of a g ?b' ?b] ide_a g apply ((auto)[1])
.
show "?thesis"
using fact2 fact3 ide_a e g Y_ide_arr "Cop.cod_char" "\<T>\<^sub>o_def" "S.comp_mkArr" preserves_arr apply ((metis)[1])
.
mqed
.
have fact5: "... = S.mkArr (Hom.set (?b, a)) (F.SET ?b')
                                 (F.FUN g o (\<lambda>x. F.FUN (\<psi> (?b, a) x) e))"
 apply ((intro S.mkArr_eqI')[1])
(*2 subgoals*)
 apply ((intro S.arr_mkArrI)[1])
(*3 subgoals*)
 apply ((metis C.ideD(1) Cop.arr_dom Cop.ide_char CopxC.arrI\<^sub>P\<^sub>C Hom.arr_map S.arr_mkIde Cop.ide_dom g Hom.map_ide ide_a)[1])
(*NEXT*)
using g apply ((force)[1])
(*NEXT*)
mproof
have fact10: "S.arr (S (\<T>\<^sub>o e ?b') (Y a g))"
using ide_a e g "\<T>\<^sub>o_in_hom" Y_ide_arr(1) "Cop.ide_char" "Cop.ide_cod" apply ((blast)[1])
.
show "?thesis"
using fact10 "1" apply ((simp)[1])
.
mqed
(*end 3 subgoals*)
(*NEXT*)
...
fix x
assume x: "x \<in> Hom.set (?b, a)"
have fact13: "((\<lambda>x. (F.FUN o \<psi> (?b', a)) x e)
                       o (\<phi> (?b', a) o Cop.comp g o \<psi> (?b, a))) x
                    = F.FUN (\<psi> (?b', a) (\<phi> (?b', a) (C (\<psi> (?b, a) x) g))) e"
 apply ((simp)[1])
.
have fact14: "... = (F.FUN g o (F.FUN o \<psi> (?b, a)) x) e"
mproof
have fact15: "\<guillemotleft>\<psi> (?b, a) x : ?b \<rightarrow> a\<guillemotright>"
using ide_a x g "Hom.\<psi>_mapsto"[of ?b a] apply ((auto)[1])
.
show "?thesis"
using fact15 assms g "Hom.\<psi>_\<phi>" "F.preserves_comp" apply ((fastforce)[1])
.
mqed
.
have fact17: "... = (F.FUN g o (\<lambda>x. F.FUN (\<psi> (?b, a) x) e)) x"
 apply ((fastforce)[1])
.
show "((\<lambda>x. F.FUN (\<psi> (?b', a) x) e) o (\<phi> (?b', a) o Cop.comp g o \<psi> (?b, a))) x
                            = (F.FUN g o (\<lambda>x. F.FUN (\<psi> (?b, a) x) e)) x"
using fact13 fact14 fact17 apply ((simp)[1])
.
mqed
(*end 2 subgoals*)
.
have fact19: "... = F g \<cdot>\<^sub>S \<T>\<^sub>o e ?b"
mproof
have fact20: "S.arr (F g) \<and> F g = S.mkArr (F.SET ?b) (F.SET ?b') (F.FUN g)"
using g "S.mkArr_Fun"[of "F g"] apply ((simp)[1])
.
have fact21: "S.arr (\<T>\<^sub>o e ?b) \<and>
               \<T>\<^sub>o e ?b = S.mkArr (Hom.set (?b, a)) (F.SET ?b) (\<lambda>x. F.FUN (\<psi> (?b, a) x) e)"
using e g "\<T>\<^sub>o_def" "\<T>\<^sub>o_in_hom" apply ((metis C.ide_cod Cop.arr_char Cop.dom_char S.in_homE)[1])
.
show "?thesis"
using fact20 fact21 "S.comp_mkArr" apply ((metis)[1])
.
mqed
.
show "\<And>x. ?thesis x"
using "1" fact5 fact19 apply ((msorry)[1])
.
mqed
(*end 2 subgoals*)
.
...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
(File.read (Path.explode "./Translator/t21.txt"))\<close>



mproof
let ?\<phi>a = "\<phi> (a, a) a"
have "\<phi>a": "\<phi> (a, a) a \<in> Hom.set (a, a)"
using ide_a "Hom.\<phi>_mapsto" apply ((fastforce)[1])
.
have 1: "\<tau> b = S.mkArr (Hom.set (b, a)) (F.SET b) (\<tau>.FUN b)"
using ide_a b "S.mkArr_Fun"[of "\<tau> b"] "Hom.set_map" apply ((auto)[1])
.
have fact0: "S.mkArr (Hom.set (b, a)) (F.SET b) (\<tau>.FUN b) = S.mkArr (Hom.set (b, a)) (F.SET b) (\<lambda>x. F.FUN (\<psi> (b, a) x) (\<tau>.FUN a (\<phi> (a, a) a)))"
 apply ((intro S.mkArr_eqI')[1])
(*2 subgoals*)
using ide_a b "1" "S.mkArr_Fun"[of "\<tau> b"] "Hom.set_map" apply ((auto)[1])
(*NEXT*)
mproof
fix x
assume x: "x \<in> Hom.set (b, a)"
...
using ide_a b x "Hom.\<psi>_mapsto"[of b a] apply ((auto)[1])
.
show "\<tau>.FUN b x = (F.FUN (\<psi> (b, a) x) (\<tau>.FUN a ?\<phi>a))"
mproof
have fact3: "\<tau>.FUN b x = S.Fun (\<tau> b \<cdot>\<^sub>S Y a ?\<psi>x) ?\<phi>a"
mproof
have fact4: "\<tau>.FUN b x = \<tau>.FUN b ((\<phi> (b, a) o Cop.comp ?\<psi>x) a)"
using ide_a b x "\<psi>x" "Hom.\<phi>_\<psi>" apply ((metis C.comp_cod_arr C.in_homE C.ide_dom Cop.comp_def comp_apply)[1])
.
have fact5: "\<tau>.FUN b ((\<phi> (b, a) \<circ> (\<cdot>\<^sup>o\<^sup>p) (\<psi> (b, a) x)) a) = (\<tau>.FUN b \<circ> (\<phi> (b, a) \<circ> (\<cdot>\<^sup>o\<^sup>p) (\<psi> (b, a) x) \<circ> \<psi> (a, a))) (\<phi> (a, a) a)"
using ide_a b "C.ide_in_hom" apply ((simp)[1])
.
have fact6: "(\<tau>.FUN b \<circ> (\<phi> (b, a) \<circ> (\<cdot>\<^sup>o\<^sup>p) (\<psi> (b, a) x) \<circ> \<psi> (a, a))) (\<phi> (a, a) a) = S.Fun (\<tau> b \<cdot>\<^sub>S Y a (\<psi> (b, a) x)) (\<phi> (a, a) a)"
mproof
have fact7: "S.seq (\<tau> b) (Y a ?\<psi>x) \<and>
                      \<tau> b \<cdot>\<^sub>S Y a ?\<psi>x =
                         S.mkArr (Hom.set (a, a)) (F.SET b)
                                 (\<tau>.FUN b o (\<phi> (b, a) \<circ> Cop.comp ?\<psi>x \<circ> \<psi> (a, a)))"
 apply ((standard)[1])
(*2 subgoals*)
using "\<psi>x" "\<tau>.is_natural_2" apply ((fastforce)[1])
(*NEXT*)
 apply ((msorry)[1])
(*end 2 subgoals*)
.
show "(\<tau>.FUN b \<circ> (\<phi> (b, a) \<circ> (\<cdot>\<^sup>o\<^sup>p) (\<psi> (b, a) x) \<circ> \<psi> (a, a))) (\<phi> (a, a) a) = S.Fun (\<tau> b \<cdot>\<^sub>S Y a (\<psi> (b, a) x)) (\<phi> (a, a) a)"
using fact7 ide_a b x "Hom.\<phi>_mapsto" "S.Fun_mkArr" apply ((force)[1])
.
mqed
.
show "\<tau>.FUN b x = S.Fun (\<tau> b \<cdot>\<^sub>S Y a (\<psi> (b, a) x)) (\<phi> (a, a) a)"
using fact4 fact5 fact6 apply ((auto)[1])
.
mqed
.
have fact12: "S.Fun (\<tau> b \<cdot>\<^sub>S Y a (\<psi> (b, a) x)) (\<phi> (a, a) a) = S.Fun (F (\<psi> (b, a) x) \<cdot>\<^sub>S \<tau> a) (\<phi> (a, a) a)"
using ide_a b "\<psi>x" "\<tau>.naturality" apply ((force)[1])
.
have fact13: "S.Fun (F (\<psi> (b, a) x) \<cdot>\<^sub>S \<tau> a) (\<phi> (a, a) a) = F.FUN (\<psi> (b, a) x) (\<tau>.FUN a (\<phi> (a, a) a))"
mproof
have fact14: "restrict (S.Fun (F ?\<psi>x \<cdot>\<^sub>S \<tau> a)) (Hom.set (a, a))
                               = restrict (F.FUN (\<psi> (b, a) x) o \<tau>.FUN a) (Hom.set (a, a))"
mproof
have fact15: "S.arr (F ?\<psi>x \<cdot>\<^sub>S \<tau> a) \<and>
                      F ?\<psi>x \<cdot>\<^sub>S \<tau> a = S.mkArr (Hom.set (a, a)) (F.SET b) (F.FUN ?\<psi>x o \<tau>.FUN a)"
 apply ((standard)[1])
(*2 subgoals*)
using "\<psi>x" ide_a "\<tau>.preserves_cod" "F.preserves_dom" apply ((elim C.in_homE)[1])
using "\<psi>x" ide_a "\<tau>.preserves_cod" "F.preserves_dom" apply ((auto)[1])
(*NEXT*)
mproof
have fact17: "\<tau> a = S.mkArr (Hom.set (a, a)) (F.SET a) (\<tau>.FUN a)"
using ide_a "1" "S.mkArr_Fun"[of "\<tau> a"] "Hom.set_map" apply ((auto)[1])
.
have fact18: "F ?\<psi>x = S.mkArr (F.SET a) (F.SET b) (F.FUN ?\<psi>x)"
using x "\<psi>x" "1" "S.mkArr_Fun"[of "F ?\<psi>x"] apply ((fastforce)[1])
.
show "F (\<psi> (b, a) x) \<cdot>\<^sub>S \<tau> a = S.mkArr (Hom.set (a, a)) (F.SET b) (F.FUN (\<psi> (b, a) x) \<circ> \<tau>.FUN a)"
 apply ((msorry)[1])
.
mqed
(*end 2 subgoals*)
.
show "restrict (S.Fun (F (\<psi> (b, a) x) \<cdot>\<^sub>S \<tau> a)) (Hom.set (a, a)) = restrict (F.FUN (\<psi> (b, a) x) \<circ> \<tau>.FUN a) (Hom.set (a, a))"
using fact15 apply ((force)[1])
.
mqed
.
show "S.Fun (F (\<psi> (b, a) x) \<cdot>\<^sub>S \<tau> a) ?\<phi>a = F.FUN ?\<psi>x (\<tau>.FUN a ?\<phi>a)"
using fact14 ide_a "\<phi>a" restr_eqE[of "S.Fun (F ?\<psi>x \<cdot>\<^sub>S \<tau> a)" "Hom.set (a, a)" "F.FUN ?\<psi>x o \<tau>.FUN a"] apply ((simp)[1])
.
mqed
.
show "\<tau>.FUN b x = F.FUN (\<psi> (b, a) x) (\<tau>.FUN a (\<phi> (a, a) a))"
using fact3 fact12 fact13 apply ((simp)[1])
.
mqed
.
mqed
(*end 2 subgoals*)
.
show "\<tau> b = S.mkArr (Hom.set (b, a)) (F.SET b) (\<lambda>x. F.FUN (\<psi> (b, a) x) (\<tau>.FUN a (\<phi> (a, a) a)))"
using "1" fact0 apply ((auto)[1])
.
mqed
.
...
mproof
interpret yoneda_lemma_fixed_e C S setp "\<phi>" F a e
using assms apply (unfold_locales, auto)
.
show "natural_transformation Cop.comp S (Y a) F (\<T> e)"
 apply (standard)
.
...
show "\<E> (\<T> e) = e"
 apply (unfold "\<E>_def")
using assms "\<T>e_ide" "S.Fun_mkArr" "Hom.\<phi>_mapsto" "Hom.\<psi>_\<phi>" ide_a "F.preserves_ide" "S.Fun_ide" restrict_apply "C.ide_in_hom" apply (auto simp add: Pi_iff)
.
mqed
(*end 2 subgoals*)
.
