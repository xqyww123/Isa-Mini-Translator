...
\qed"\<close>
 ...
 apply (insert assms(1), induction x rule: less_induct)
mproof
case less:   (less x)
show "P x"
using assms step_less step_ge_x0 less apply (cases "x < x\<^sub>1")
 apply (auto)
.
mqed
.
...
mproof
have fact0: "((\<lambda>x. hb * ln x powr -(1+e)) \<longlongrightarrow> 0) at_top"
using e_pos apply ((intro tendsto_mult_right_zero tendsto_neg_powr ln_at_top)[1])
 apply ((simp)[1])
.
have fact1: "\<forall>b\<in>set bs. eventually (\<lambda>x. \<bar>hb * ln x powr -(1+e)\<bar> < b - C) at_top"
using assms fact0 apply ((force simp: tendsto_iff dist_real_def)[1])
.
have fact2: "eventually (\<lambda>x. \<forall>b\<in>set bs. \<bar>hb * ln x powr -(1+e)\<bar> < b - C) at_top"
using fact1 apply ((intro eventually_ball_finite)[1])
(*2 subgoals*)
 apply ((simp)[1])
(*NEXT*)
 apply ((simp)[1])
...
note A = eventually_conj[OF this eventually_gt_at_top]
show "?thesis"
using A apply ((eventually_elim)[1])
 apply ((clarify)[1])
mproof
fix x :: real and b :: real
assume x: "x > 0" and b: "b \<in> set bs"
assume A: "\<forall>b\<in>set bs. \<bar>hb * ln x powr -(1+e)\<bar> < b - C"
have fact4: "hb * ln x powr -(1+e) < b - C"
using b A assms apply ((msorry)[1])
.
have fact5: "x * (hb * ln x powr -(1+e)) < x * (b - C)"
using x fact4 apply ((intro mult_strict_left_mono)[1])
(*2 subgoals*)
 apply ((assumption)[1])
(*NEXT*)
.
(*end 2 subgoals*)
show "C*x \<le> b*x - hb*x / ln x powr (1+e)"
using fact5 apply ((subst (asm) powr_minus)[1])
 apply ((simp add: field_simps)[1])
.
mqed
.
mqed
