...
theory MS_Test_SmallStep
imports
  PCF.OpSem Minilang_Translator.MS_Translator
begin
...
"using assms by (induct arbitrary: t'') (blast dest: reduction_val elim: reduction_inv)+" \<close>
...
lemma reduction_eval:
  ...
  shows "t \<Down> v"
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state} 
"using assms by (induct arbitrary: v) (auto elim!: evalOP_inv val.cases intro: eval_val)" \<close>
