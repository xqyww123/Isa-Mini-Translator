...
theory MS_Test_Deutsch
imports
  Isabelle_Marries_Dirac.Deutsch
  Isabelle_Marries_Dirac.More_Tensor
  Isabelle_Marries_Dirac.Binary_Nat MS_Translator
begin
...
(File.read (Path.explode "./Translator/t37.txt"))\<close>
...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state}
(File.read (Path.explode "./Translator/t38.txt"))\<close>
...
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state}
(File.read (Path.explode "./Translator/t39.txt"))\<close>
...
lemma (in jozsa) jozsa_transform_is_unitary_index_odd:
  ...
  shows "(U\<^sub>f * U\<^sub>f) $$ (i,j) = 1\<^sub>m (dim_col U\<^sub>f) $$ (i,j)"
ML_val \<open>val _ =  MinLang_Translator.translate'm  @{Isar.state}
(File.read (Path.explode "./Translator/t40.txt"))\<close>
